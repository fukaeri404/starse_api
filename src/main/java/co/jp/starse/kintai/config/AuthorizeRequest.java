...
public class AuthorizeRequest {
	...
	static void configureAuthorizeRequests(
		...
		request.requestMatchers(HttpMethod.POST, Routes.LOGIN).permitAll()
				.requestMatchers(HttpMethod.POST, Routes.REGISTER).hasAnyAuthority("1").requestMatchers(Routes.Admin)
				.hasAnyAuthority("1").requestMatchers(Routes.User, Routes.CHANGE_PASSWORD)
				.hasAnyAuthority("1", "2", "3", "4").requestMatchers(Routes.PUBLIC, Routes.FORGOT_PASSWORD).permitAll()
